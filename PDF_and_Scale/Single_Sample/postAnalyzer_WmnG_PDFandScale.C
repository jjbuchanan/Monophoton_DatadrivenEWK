...
#include "postAnalyzer_WlnG_mc_znng.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
    ...
    //    nHLTPassed++;
          if(phoCand1.size() >0)
          {
            ...
            if( TMath::Max( ( (*phoPFChWorstIso)[phoCand1[0]]  - rho*EAchargedworst((*phoSCEta)[phoCand1[0]]) ), 0.0) < 1.37 )
            {
              ...
              if(mulist.size() == 1 && looseMus.size() == 1 && looseEles.size() == 0)
              {
                ...
                if(leptoMET > 170)
                {
                  ...
                  {
                    passed170+=event_weight;
                    fillHistos(110,event_weight,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    //PDF
                    central_sum_passing += event_weight*lheNormalizedWeights->at(0);
                    for(unsigned int i = 0; i < nMCreplicas; i++)
                    {
                      sum_passing[i] += event_weight*lheNormalizedWeights->at(vecIndices[i]);
                      fillHistos(i,event_weight*lheNormalizedWeights->at(vecIndices[i]),phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    }
                    //Scale
                    sum170_QCDscale_muR1_muF1 += event_weight*genWeight_QCDscale_muR1_muF1;
                    sum170_QCDscale_muR1_muF2 += event_weight*genWeight_QCDscale_muR1_muF2;
                    sum170_QCDscale_muR1_muF0p5 += event_weight*genWeight_QCDscale_muR1_muF0p5;
                    sum170_QCDscale_muR2_muF1 += event_weight*genWeight_QCDscale_muR2_muF1;
                    sum170_QCDscale_muR2_muF2 += event_weight*genWeight_QCDscale_muR2_muF2;
                    sum170_QCDscale_muR2_muF0p5 += event_weight*genWeight_QCDscale_muR2_muF0p5;
                    sum170_QCDscale_muR0p5_muF1 += event_weight*genWeight_QCDscale_muR0p5_muF1;
                    sum170_QCDscale_muR0p5_muF2 += event_weight*genWeight_QCDscale_muR0p5_muF2;
                    sum170_QCDscale_muR0p5_muF0p5 += event_weight*genWeight_QCDscale_muR0p5_muF0p5;
                    fillHistos(nMCreplicas,event_weight*genWeight_QCDscale_muR1_muF1,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+1,event_weight*genWeight_QCDscale_muR1_muF2,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+2,event_weight*genWeight_QCDscale_muR1_muF0p5,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+3,event_weight*genWeight_QCDscale_muR2_muF1,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+4,event_weight*genWeight_QCDscale_muR2_muF2,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+5,event_weight*genWeight_QCDscale_muR2_muF0p5,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+6,event_weight*genWeight_QCDscale_muR0p5_muF1,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+7,event_weight*genWeight_QCDscale_muR0p5_muF2,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                    fillHistos(nMCreplicas+8,event_weight*genWeight_QCDscale_muR0p5_muF0p5,phoCand1[0],jetveto,mulist,leptoMET,leptoMET_phi);
                  }
                }
              }
            }
